...
*/
INSERT INTO [history].[AzDataFactoryV2Trigger]
([ETag], [DataFactoryName], [Name], [RuntimeState], [ResourceGroupName], [RecordCreationDate])
SELECT st.[Etag], st.[DataFactoryName], st.[Name], st.[RuntimeState], st.[ResourceGroupName], GETDATE()
FROM [history].[AzDataFactoryV2Trigger] AS ht
RIGHT OUTER JOIN [staging].[AzDataFactoryV2Trigger] AS st
ON ht.[ETag] = st.[ETag]
...
FROM [staging].[AzDataFactoryV2Trigger] AS s
WHERE EXISTS (
	SELECT [ETag], [DataFactoryName], [Name], [ResourceGroupName]
	FROM [history].[AzDataFactoryV2Trigger] AS h
	WHERE s.[ETag] = h.[Etag]
	AND s.[Name] <> h.[Name])

INSERT INTO [history].[AzDataFactoryV2Trigger] 
([ETag], [DataFactoryName], [Name], [RuntimeState], [ResourceGroupName])
SELECT [ETag], [DataFactoryName], [Name], [RuntimeState], [ResourceGroupName]
FROM [staging].[AzDataFactoryV2Trigger] AS s
WHERE EXISTS (
	SELECT [ETag], [DataFactoryName], [Name], [ResourceGroupName]
	FROM [history].[AzDataFactoryV2Trigger] AS h
	WHERE s.[ETag] = h.[Etag]
	AND s.[RuntimeState] <> h.[RuntimeState])
...
*/
UPDATE [history].[AzDataFactoryV2Trigger] 
SET [HistoricalName] = ht.[Name],
[history].[AzDataFactoryV2Trigger].[Name] = st.[Name],
[IsCurrent] = 1,
[HistoricalNameDate] = getdate()
FROM [history].[AzDataFactoryV2Trigger] AS ht
JOIN [staging].[AzDataFactoryV2Trigger] AS st
ON ht.[ETag] = st.[ETag]
WHERE ht.[Name] <> st.[Name]

UPDATE [history].[AzDataFactoryV2Trigger] 
SET [HistoricalRuntimeState] = ht.[RuntimeState],
[history].[AzDataFactoryV2Trigger].[RuntimeState] = st.[RuntimeState],
[IsCurrent] = 1,
[HistoricalRuntimeStateDate] = getdate()
FROM [history].[AzDataFactoryV2Trigger] AS ht
JOIN [staging].[AzDataFactoryV2Trigger] AS st
ON ht.[ETag] = st.[ETag]
WHERE ht.[RuntimeState] <> st.[RuntimeState] 
...
UPDATE [history].[AzDataFactoryV2Trigger] 
SET [IsCurrent] = 1
WHERE [HistoricalRuntimeState] = [RuntimeState]
...
UPDATE [history].[AzDataFactoryV2Trigger] 
SET [IsCurrent] = 1
WHERE [HistoricalRuntimeState] IS NULL
...
[RecordEndDate] = getdate()
WHERE [HistoricalRuntimeState] <> [RuntimeState]
AND [HistoricalRuntimeState] IS NOT NULL
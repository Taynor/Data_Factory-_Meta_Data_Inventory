...
*/
INSERT INTO [history].[AzDataFactoryV2LinkedService]
...
FROM [history].[AzDataFactoryV2LinkedService] AS ht
RIGHT OUTER JOIN [staging].[AzDataFactoryV2LinkedService] AS st
ON ht.[ETag] = st.[ETag]
...
FROM [staging].[AzDataFactoryV2LinkedService] AS s
WHERE EXISTS (
	SELECT [ETag], [DataFactoryName], [Name], [ResourceGroupName]
	FROM [history].[AzDataFactoryV2LinkedService] AS h
	WHERE s.[ETag] = h.[Etag]
	AND s.[Name] <> h.[Name])
...
*/
UPDATE [history].[AzDataFactoryV2LinkedService] 
SET [HistoricalName] = ht.[Name],
[history].[AzDataFactoryV2LinkedService].[Name] = st.[Name],
[IsCurrent] = 1,
[HistoricalNameDate] = getdate()
FROM [history].[AzDataFactoryV2LinkedService] AS ht
JOIN [staging].[AzDataFactoryV2LinkedService] AS st
ON ht.[ETag] = st.[ETag]

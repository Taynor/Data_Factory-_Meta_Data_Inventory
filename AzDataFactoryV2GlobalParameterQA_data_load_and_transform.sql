...
*/
INSERT INTO [history].[AzDataFactoryV2GlobalParametersQA]
([GlobalParameterKey], [GlobalParameterValue])
SELECT st.[GlobalParameterKey], st.[GlobalParameterValue], GETDATE()
FROM [history].[AzDataFactoryV2GlobalParametersQA] AS ht
RIGHT OUTER JOIN [staging].[AzDataFactoryV2GlobalParametersQA] AS st
...
FROM [staging].[AzDataFactoryV2GlobalParametersQA] AS s
WHERE EXISTS (
	SELECT [GlobalParameterKey], [GlobalParameterValue]
	FROM [history].[AzDataFactoryV2GlobalParametersQA] AS h
...
*/
UPDATE [history].[AzDataFactoryV2GlobalParametersQA] 
SET [HistoricalGlobalParameterValue] = ht.[GlobalParameterValue],
[history].[AzDataFactoryV2GlobalParametersQA].[GlobalParameterValue] = st.[GlobalParameterValue],
[IsCurrent] = 1,
[HistoricalGlobalParameterValueDate] = getdate()
FROM [history].[AzDataFactoryV2GlobalParametersQA] AS ht
JOIN [staging].[AzDataFactoryV2GlobalParametersQA] AS st
ON ht.[GlobalParameterKey] = st.[GlobalParameterKey]
...
WHERE [RecordCreationDate] = (SELECT MAX([RecordCreationDate]) 
							FROM [history].[AzDataFactoryV2GlobalParametersQA])

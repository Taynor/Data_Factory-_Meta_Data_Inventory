...
*/
INSERT INTO [history].[AzDatafactoryV2Pipeline]
...
FROM [history].[AzDatafactoryV2Pipeline] AS ht
RIGHT OUTER JOIN [staging].[AzDatafactoryV2Pipeline] AS st
ON ht.[ETag] = st.[ETag]
...
FROM [staging].[AzDatafactoryV2Pipeline] AS s
WHERE EXISTS (
	SELECT [ETag], [DataFactoryName], [Name], [ResourceGroupName]
	FROM [history].[AzDatafactoryV2Pipeline] AS h
	WHERE s.[ETag] = h.[Etag]
	AND s.[Name] <> h.[Name])
...
*/
UPDATE [history].[AzDatafactoryV2Pipeline] 
SET [HistoricalName] = ht.[Name],
[history].[AzDatafactoryV2Pipeline].[Name] = st.[Name],
[IsCurrent] = 1,
[HistoricalNameDate] = getdate()
FROM [history].[AzDatafactoryV2Pipeline] AS ht
JOIN [staging].[AzDatafactoryV2Pipeline] AS st
ON ht.[ETag] = st.[ETag]

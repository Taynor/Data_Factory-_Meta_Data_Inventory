...
$datafactoryGlobalParametersArrayDevelopment = for ($i = 0; $i -lt $MaxArraySizeDevelopment; $i++){ 
    Write-Verbose "$($keyContentDevelopment[$i]),$($valueContentDevelopment[$i])"
    [PSCustomObject]@{
...
$datafactoryGlobalParametersArrayDevelopment | `
Write-SqlTableData -ServerInstance $targetSqlInstance -DatabaseName $targetDatabase -Credential $Credential -SchemaName $targetSchema -TableName 'AzDataFactoryV2GlobalParametersDevelopment' -Force
...
$datafactoryGlobalParametersArrayQA = for ($i = 0; $i -lt $MaxArraySizeQA; $i++){ 
    Write-Verbose "$($keyContentQA[$i]),$($valueContentQA[$i])"
    [PSCustomObject]@{
        GlobalParameterKey = $keyContentQA[$i]
        GlobalParameterValue = $valueContentQA[$i]
    }
}

$datafactoryGlobalParametersArrayQA | `
Write-SqlTableData -ServerInstance $targetSqlInstance -DatabaseName $targetDatabase -Credential $Credential -SchemaName $targetSchema -TableName 'AzDataFactoryV2GlobalParametersQA' -Force
...
$datafactoryGlobalParametersArrayProduction = for ($i = 0; $i -lt $MaxArraySizeProduction; $i++){ 
    Write-Verbose "$($keyContentProduction[$i]),$($valueContentProduction[$i])"
    [PSCustomObject]@{
...
$datafactoryGlobalParametersArrayProduction | `
Write-SqlTableData -ServerInstance $targetSqlInstance -DatabaseName $targetDatabase -Credential $Credential -SchemaName $targetSchema -TableName 'AzDataFactoryV2GlobalParametersProduction' -Force
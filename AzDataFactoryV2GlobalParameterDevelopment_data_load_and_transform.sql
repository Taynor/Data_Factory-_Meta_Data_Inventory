...
*/
INSERT INTO [history].[AzDataFactoryV2GlobalParametersDevelopment]
([GlobalParameterKey], [GlobalParameterValue])
SELECT st.[GlobalParameterKey], st.[GlobalParameterValue], GETDATE()
FROM [history].[AzDataFactoryV2GlobalParametersDevelopment] AS ht
RIGHT OUTER JOIN [staging].[AzDataFactoryV2GlobalParametersDevelopment] AS st
...
FROM [staging].[AzDataFactoryV2GlobalParametersDevelopment] AS s
WHERE EXISTS (
	SELECT [GlobalParameterKey], [GlobalParameterValue]
	FROM [history].[AzDataFactoryV2GlobalParametersDevelopment] AS h
...
*/
UPDATE [history].[AzDataFactoryV2GlobalParametersDevelopment] 
SET [HistoricalGlobalParameterValue] = ht.[GlobalParameterValue],
[history].[AzDataFactoryV2GlobalParametersDevelopment].[GlobalParameterValue] = st.[GlobalParameterValue],
[IsCurrent] = 1,
[HistoricalGlobalParameterValueDate] = getdate()
FROM [history].[AzDataFactoryV2GlobalParametersDevelopment] AS ht
JOIN [staging].[AzDataFactoryV2GlobalParametersDevelopment] AS st
ON ht.[GlobalParameterKey] = st.[GlobalParameterKey]
...
WHERE [RecordCreationDate] = (SELECT MAX([RecordCreationDate]) 
							FROM [history].[AzDataFactoryV2GlobalParametersDevelopment])

...
*/
INSERT INTO [history].[AzDataFactoryV2DataFlow] 
...
FROM [history].[AzDataFactoryV2DataFlow] AS ht
RIGHT OUTER JOIN [staging].[AzDataFactoryV2DataFlow] AS st
ON ht.[ETag] = st.[ETag]
...
FROM [staging].[AzDataFactoryV2DataFlow] AS s
WHERE EXISTS (
	SELECT [ETag], [DataFactoryName], [Name], [ResourceGroupName]
	FROM [history].[AzDataFactoryV2DataFlow] AS h
	WHERE s.[ETag] = h.[Etag]
	AND s.[Name] <> h.[Name])
...
*/
UPDATE [history].[AzDataFactoryV2DataFlow] 
SET [HistoricalName] = ht.[Name],
[history].[AzDataFactoryV2DataFlow].[Name] = st.[Name],
[IsCurrent] = 1,
[HistoricalNameDate] = getdate()
FROM [history].[AzDataFactoryV2DataFlow] AS ht
JOIN [staging].[AzDataFactoryV2DataFlow] AS st
ON ht.[ETag] = st.[ETag]
...
WHERE [RecordCreationDate] = (SELECT MAX([RecordCreationDate]) 
							FROM [history].[AzDataFactoryV2DataFlow])
